...
describe('Testing adminQuizTrashEmpty', () => {
  describe('Error Tests', () => {
    test('Trash is already empty', () => {
      ...
      const emptyTrash = v2requestAdminQuizTrashEmpty(user.token, [quizId.quizId]);
    ...
    test('Quiz is not in the trash', () => {
      ...
      const emptyTrash = v2requestAdminQuizTrashEmpty(user.token, [quiz2Id.quizId]);
      expect(emptyTrash.body).toEqual(error);
      expect(emptyTrash.statusCode).toEqual(400);
    });
    test('QuizId is invalid', () => {
      ...
      const emptyTrash = v2requestAdminQuizTrashEmpty(user.token, [quizId.quizId + 1]);
      expect(emptyTrash.body).toEqual(error);
    ...
    test('Token is invalid', () => {
      ...
      const emptyTrash = v2requestAdminQuizTrashEmpty((user.token + 'invalid'), [quizId.quizId]);
      expect(emptyTrash.body).toEqual(error);
    ...
    test('Token is empty', () => {
      ...
      const emptyTrash = v2requestAdminQuizTrashEmpty('', [quizId.quizId]);
      expect(emptyTrash.body).toEqual(error);
      expect(emptyTrash.statusCode).toEqual(401);
    });
    test('User does not own this quiz', () => {
      ...
      const emptyTrash = v2requestAdminQuizTrashEmpty(user2.token, [quizId.quizId]);
      expect(emptyTrash.body).toEqual(error);
      expect(emptyTrash.statusCode).toEqual(403);
    });
  });
  describe('Passing Tests', () => {
    ...
    test('2 quizes successfullly emptied from the trash', () => {
      ...
      const quizId = v2requestAdminQuizCreate(user.token, 'Aero', 'Quiz about Aeroplanes').body;
      const quiz2Id = v2requestAdminQuizCreate(user.token, 'Space', 'Quiz about Space').body;
      const quiz3Id = v2requestAdminQuizCreate(user.token, 'Dino', 'Quiz about Dinos').body;
      // Remove 2 quizzes
      v2requestAdminQuizRemove(user.token, quizId.quizId);
      v2requestAdminQuizRemove(user.token, quiz2Id.quizId);
      // Empty both quizzes from trash
      const emptyTrash = v2requestAdminQuizTrashEmpty(user.token, [quizId.quizId, quiz2Id.quizId]);
      expect(emptyTrash.body).toEqual({});
      expect(emptyTrash.statusCode).toEqual(200);
      expect(v2requestAdminQuizList(user.token).body).toEqual({
        quizzes: [{
