import {
  requestAdminQuizCreate, requestAdminAuthRegister, requestAdminQuizTransfer,
  requestAdminQuestionCreate, requestClear
...
describe('adminQuizTransfer', () => {
  describe('Error Tests', () => {
    test('Token is invalid', () => {
      ...
      const transfer = requestAdminQuizTransfer(quizId.quizId, 'invalidToken', 'isaacchiu@gmail.com');
    ...
    test('Token is empty', () => {
      ...
      requestAdminAuthRegister('isaacchiu@gmail.com', 'Password1', 'Isaac', 'Chiu');
      const quizId = requestAdminQuizCreate(user.token, 'Aero', 'Quiz about Aeroplanes').body;
      const transfer = requestAdminQuizTransfer(quizId.quizId, '', 'isaacchiu@gmail.com');
      expect(transfer.body).toEqual(error);
      expect(transfer.statusCode).toEqual(401);
    });
    test('QuizId is invalid', () => {
      ...
      requestAdminAuthRegister('isaacchiu@gmail.com', 'Password1', 'Isaac', 'Chiu');
      const quizId = requestAdminQuizCreate(user.token, 'Aero', 'Quiz about Aeroplanes').body;
      const transfer = requestAdminQuizTransfer(quizId.quizId + 1, user.token, 'isaacchiu@gmail.com');
      expect(transfer.body).toEqual(error);
    ...
    test('QuizId belongs to another user', () => {
      ...
      const quizId = requestAdminQuizCreate(user.token, 'Aero', 'Quiz about Aeroplanes').body;
      const transfer = requestAdminQuizTransfer(quizId.quizId, user2.token, 'isaacchiu@gmail.com');
      expect(transfer.body).toEqual(error);
      expect(transfer.statusCode).toEqual(403);
    });
    test('userEmail is not a real user', () => {
      const user = requestAdminAuthRegister('matthewvuci@gmail.com', 'Password1', 'Matthew', 'Vucic').body;
      const quizId = requestAdminQuizCreate(user.token, 'Aero', 'Quiz about Aeroplanes').body;
      const transfer = requestAdminQuizTransfer(quizId.quizId, user.token, 'isaacchiu@gmail.com');
      expect(transfer.body).toEqual(error);
    ...
    test('userEmail is the logged in user', () => {
      const user = requestAdminAuthRegister('matthewvuci@gmail.com', 'Password1', 'Matthew', 'Vucic').body;
      const quizId = requestAdminQuizCreate(user.token, 'Aero', 'Quiz about Aeroplanes').body;
      const transfer = requestAdminQuizTransfer(quizId.quizId, user.token, 'matthewvuci@gmail.com');
      expect(transfer.body).toEqual(error);
      expect(transfer.statusCode).toEqual(400);
    });
    test('Quiz ID refers to a quiz that has a name that is already used by the target user', () => {
      ...
      const user2 = requestAdminAuthRegister('isaacchiu@gmail.com', 'Password1', 'Isaac', 'Chiu').body;
      const quizId = requestAdminQuizCreate(user.token, 'Aero', 'Quiz about Aeroplanes').body;
      requestAdminQuizCreate(user2.token, 'Aero', 'Quiz about Aeroplanes');
      const transfer = requestAdminQuizTransfer(quizId.quizId, user.token, 'isaacchiu@gmail.com');
      expect(transfer.body).toEqual(error);
      expect(transfer.statusCode).toEqual(400);
    });
  ...
  describe('Passing Tests', () => {
    ...
    test('Multiple quizzes are transfered sucessfully', () => {
      ...
      requestAdminAuthRegister('isaacchiu@gmail.com', 'Password1', 'Isaac', 'Chiu');
      const quizId = requestAdminQuizCreate(user.token, 'Aero', 'Quiz about Aeroplanes').body;
      const quizId2 = requestAdminQuizCreate(user.token, 'Plane', 'Quiz about Aeroplanes').body;
      const transfer = requestAdminQuizTransfer(quizId.quizId, user.token, 'isaacchiu@gmail.com');
      const transfer2 = requestAdminQuizTransfer(quizId2.quizId, user.token, 'isaacchiu@gmail.com');
      expect(transfer.body).toEqual({});
      ...
      const question = requestAdminQuestionCreate(quizId.quizId, user.token, questionBody);
      const question2 = requestAdminQuestionCreate(quizId2.quizId, user.token, questionBody);
      expect(question.body).toEqual(error);
