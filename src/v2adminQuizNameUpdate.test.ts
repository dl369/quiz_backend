import {
  ...
  requestClear, v2requestAdminQuizNameUpdate
...
describe('adminQuizInfo', () => {
  describe('Error Tests', () => {
    test('Invalid token', () => {
      ...
      const quizNameUpdate = v2requestAdminQuizNameUpdate(user.token + 'invalid', quiz.quizId, 'new quiz');
    ...
    test('Invalid quizId', () => {
      ...
      const quizNameUpdate = v2requestAdminQuizNameUpdate(user.token, quiz.quizId + 1, 'new quiz');
      expect(quizNameUpdate.body).toStrictEqual(error);
    ...
    test('Quiz not owned by user', () => {
      ...
      const quizNameUpdate = v2requestAdminQuizNameUpdate(user2.token, quiz.quizId, 'new quiz');
      expect(quizNameUpdate.body).toStrictEqual(error);
      expect(quizNameUpdate.statusCode).toStrictEqual(403);
    });
    test('Name contains invalid characters', () => {
      ...
      const quizNameUpdate = v2requestAdminQuizNameUpdate(user.token, quiz.quizId, 'new quiz$$$');
      expect(quizNameUpdate.body).toStrictEqual(error);
    ...
    test('Name is less than 3 characters long', () => {
      ...
      const quizNameUpdate = v2requestAdminQuizNameUpdate(user.token, quiz.quizId, '1');
      expect(quizNameUpdate.body).toStrictEqual(error);
      expect(quizNameUpdate.statusCode).toStrictEqual(400);
    });
    test('Name is greater than 30 characters long', () => {
      ...
      const quizNameUpdate = v2requestAdminQuizNameUpdate(user.token, quiz.quizId, 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa');
      expect(quizNameUpdate.body).toStrictEqual(error);
      expect(quizNameUpdate.statusCode).toStrictEqual(400);
    });
    test('Name is already used by the current logged in user for another quiz', () => {
      const user = requestAdminAuthRegister('matthewvucic@gmail.com', 'Password1', 'Matthew', 'Vucic').body;
      const quiz = v2requestAdminQuizCreate(user.token, 'Aero', 'Quiz about Aeroplanes').body;
      v2requestAdminQuizCreate(user.token, 'Aero2', 'Quiz about Aeroplanes');

      const quizNameUpdate = v2requestAdminQuizNameUpdate(user.token, quiz.quizId, 'Aero2');
      expect(quizNameUpdate.body).toStrictEqual(error);
      expect(quizNameUpdate.statusCode).toStrictEqual(400);
    });
  });
  describe('Passing Tests', () => {
    ...
    test('update quiz name to same name', () => {
      const user = requestAdminAuthRegister('matthewvucic@gmail.com', 'Password1', 'Matthew', 'Vucic').body;
      const quiz = v2requestAdminQuizCreate(user.token, 'My Quiz', 'This is my quiz').body;
      const quizNameUpdate = v2requestAdminQuizNameUpdate(user.token, quiz.quizId, 'My Quiz');
      expect(quizNameUpdate.body).toStrictEqual({});
      const quizInfo = v2requestAdminQuizInfo(user.token, quiz.quizId);
      expect(quizInfo.body).toStrictEqual({
        quizId: quiz.quizId,
