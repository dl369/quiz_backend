import { requestAdminAuthRegister, v2requestAdminUserPasswordUpdate, requestClear } from './requestBoilerPlates';
...
describe('Admin User Password Update Tests', () => {
  test('Rejects password update when old and new passwords are identical', () => {
    ...
    const passwordUpdateResponse = v2requestAdminUserPasswordUpdate(token, 'Password1', 'Password1');
  ...
  test('Rejects password update when the new password was previously used', () => {
    ...
    v2requestAdminUserPasswordUpdate(token, 'Password1', 'Password12');
    const passwordUpdateResponse = v2requestAdminUserPasswordUpdate(token, 'Password12', 'Password1');
  ...
  test('Rejects password update for passwords under 8 characters', () => {
    ...
    const passwordUpdateResponse = v2requestAdminUserPasswordUpdate(token, 'Password1', 'short');
  ...
  test('Rejects password update without a mix of letters and numbers', () => {
    ...
    const responseLettersOnly = v2requestAdminUserPasswordUpdate(
      token,
      'Password1',
      'OnlyLetters!'
    );
    ...
    const responseNumbersOnly = v2requestAdminUserPasswordUpdate(
      token,
      'Password1',
      '123456789'
    );
  ...
  test('should reject password update due to old password mismatch', () => {
    ...
    const result = v2requestAdminUserPasswordUpdate(
      token,
