import {
  requestAdminQuizQuestionMove, requestAdminAuthRegister,
  requestAdminQuizCreate, requestAdminQuestionCreate, requestAdminQuizInfo,
  requestClear
...
describe('Admin Quiz Question Move Tests', () => {
  test('Successfully moves a quiz question', () => {
    ...
    const question2 = requestAdminQuestionCreate(quiz.quizId, user.token, {
    ...
    const response = requestAdminQuizQuestionMove(quiz.quizId, question2.questionId, user.token, 0);
  ...
  test('Rejects move if Question Id does not refer to a valid question within this quiz', () => {
    ...
    const moveResponse = requestAdminQuizQuestionMove(quiz.quizId, quiz.quizId + 1, user.token, newPosition);
  ...
  test('Rejects move if NewPosition is out of valid range', () => {
    ...
    const response = requestAdminQuizQuestionMove(quiz.quizId, question.questionId, user.token, invalidNewPosition);
  ...
  test('Rejects move if NewPosition is the same as the current position', () => {
    ...
    const response = requestAdminQuizQuestionMove(quiz.quizId, question.questionId, user.token, 0);
  ...
  test('Rejects move for invalid token (401 Unauthorized)', () => {
    ...
    const questionId = requestAdminQuestionCreate(quizId.quizId, user.token, questionBody).body;
    const newPosition = 2; // New position to move the question to
    const invalidToken = user.token + 'invalid'; // Simulated invalid token

    const response = requestAdminQuizQuestionMove(quizId.quizId, questionId.questionId, invalidToken, newPosition);
  ...
  test('Invalid quizId', () => {
    ...
    const newPosition = 2;
    const question = requestAdminQuestionCreate(quiz.quizId, user.token, questionBody).body;
    const response = requestAdminQuizQuestionMove(quiz.quizId + 1, question.questionId, user.token, newPosition);
  ...
  test('Quiz not owned by user', () => {
    ...
    const newPosition = 2;
    const question = requestAdminQuestionCreate(quiz.quizId, user.token, questionBody).body;
    const response = requestAdminQuizQuestionMove(quiz.quizId, question.questionId, user2.token, newPosition);
