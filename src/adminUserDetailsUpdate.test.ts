...
describe('Testing adminUserDetailsUpdate', () => {
  describe('Error Tests:', () => {
    test('Token is Invalid', () => {
      ...
      const detailsUpdate = requestAdminUserDetailsUpdate(user.token + 'invalid', 'z1234567@ad.unsw.edu.au', 'Bob', 'Johnson');
      expect(detailsUpdate.body).toStrictEqual(error);
      expect(detailsUpdate.statusCode).toStrictEqual(401);
    });
    test('Token is Empty', () => {
      const detailsUpdate = requestAdminUserDetailsUpdate('', 'z1234567@ad.unsw.edu.au', 'Bob', 'Johnson');
      expect(detailsUpdate.body).toStrictEqual(error);
      expect(detailsUpdate.statusCode).toStrictEqual(401);
    });
    test('Email is currently being used by another user', () => {
      ...
      const detailsUpdate = requestAdminUserDetailsUpdate(user.token, 'darrius@gmail.com', 'Bob', 'Johnson');
      expect(detailsUpdate.body).toStrictEqual(error);
    ...
    test.each([
      ...
      const user = requestAdminAuthRegister('matthewvucic@gmail.com', 'Password1', 'Matthew', 'Vucic').body;
      const userDetailsUpdate = requestAdminUserDetailsUpdate(user.token, email, 'Bob', 'Johnson');
    ...
    test.each([
      ...
      const user = requestAdminAuthRegister('matthewvucic@gmail.com', 'Password1', 'Matthew', 'Vucic').body;
      const userDetailsUpdate = requestAdminUserDetailsUpdate(user.token, 'victor@gmail.com', nameFirst, 'Johnson');
      expect(userDetailsUpdate.body).toStrictEqual(error);
      expect(userDetailsUpdate.statusCode).toStrictEqual(400);
    });
    ...
    test.each([
      ...
      const user = requestAdminAuthRegister('matthewvucic@gmail.com', 'Password1', 'Matthew', 'Vucic').body;
      const userDetailsUpdate = requestAdminUserDetailsUpdate(user.token, 'victor@gmail.com', 'Bob', nameLast);
      expect(userDetailsUpdate.body).toStrictEqual(error);
      expect(userDetailsUpdate.statusCode).toStrictEqual(400);
    });
  });
  test('AdminUserDetailsUpdate is successful', () => {
    const user = requestAdminAuthRegister('matthewvucic@gmail.com', 'Password1', 'Matthew', 'Vucic').body;
    const userDetailsUpdate = requestAdminUserDetailsUpdate(user.token, 'expectedvictor@gmail.com', 'Bob', 'Johnson');
    expect(userDetailsUpdate.body).toStrictEqual({});
    expect(userDetailsUpdate.statusCode).toStrictEqual(200);
    expect(requestAdminUserDetails(user.token).body).toStrictEqual({
      user: {
  ...
  test('update user to same details', () => {
    const user = requestAdminAuthRegister('matthewvucic@gmail.com', 'Password1', 'Matthew', 'Vucic').body;
    const userDetailsUpdate = requestAdminUserDetailsUpdate(user.token, 'matthewvucic@gmail.com', 'Matthew', 'Vucic');
    expect(userDetailsUpdate.body).toStrictEqual({});
    expect(requestAdminUserDetails(user.token).body).toStrictEqual({
      user: {
        userId: expect.any(Number),
