import { requestAdminAuthRegister, v2requestAdminQuizCreate, v2requestAdminQuizInfo, requestClear } from './requestBoilerPlates';
...
describe('adminQuizInfo', () => {
  describe('Error Tests', () => {
    test('Invalid token', () => {
      ...
      const quizInfo = v2requestAdminQuizInfo(user.token + 'invalid', quiz.quizId);
    ...
    test('Invalid quizId', () => {
      ...
      const quizInfo = v2requestAdminQuizInfo(user.token, quiz.quizId + 1);
      expect(quizInfo.body).toStrictEqual(error);
    ...
    test('Quiz not owned by user', () => {
      ...
      const quizInfo = v2requestAdminQuizInfo(user2.token, quiz.quizId);
      expect(quizInfo.body).toStrictEqual(error);
      expect(quizInfo.statusCode).toStrictEqual(403);
    });
  });
