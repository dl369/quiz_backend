...
describe('AnswerSubmit', () => {
  describe('Error Tests', () => {
    test('Invalid sessionId', () => {
      ...
      const answerResult = requestQuizSessionFinalResultsCsv(user.token, quiz.quizId, session.sessionId + 1);
    ...
    test('Invalid quizId', () => {
      ...
      const answerResult = requestQuizSessionFinalResultsCsv(user.token, quiz.quizId + 1, session.sessionId);
      expect(answerResult.body).toEqual(error);
    ...
    test('invalid session state', () => {
      ...
      const answerResult = requestQuizSessionFinalResultsCsv(user.token, quiz.quizId, session.sessionId);
      expect(answerResult.body).toEqual(error);
      expect(answerResult.statusCode).toStrictEqual(400);
    });
    test('Invalid token', () => {
      ...
      const answerResult = requestQuizSessionFinalResultsCsv('Invalid Token', quiz.quizId, session.sessionId);
      expect(answerResult.body).toEqual(error);
    ...
    test('Valid token provided but not owner of quiz', () => {
      ...
      const answerResult = requestQuizSessionFinalResultsCsv(user1.token, quiz.quizId, session.sessionId);
      expect(answerResult.body).toEqual(error);
      expect(answerResult.statusCode).toStrictEqual(403);
    });
  });
  ...
  describe('Passing Tests', () => {
    test('1 correct answer, multiple player', () => {
      ...
      expect(answerResult.body).toStrictEqual({ url: expect.any(String) });
      expect(answerResult.statusCode).toStrictEqual(200);
    });
  });
});
