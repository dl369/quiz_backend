import {
  requestAdminAuthRegister, requestAdminAuthLogin, requestAdminAuthLogout,
...
describe('adminAuthLogout HTTP Tests', () => {
  describe('Logout successful', () => {
    test('Valid logged-in token input', () => {
      ...
      const logOut1 = requestAdminAuthLogout(token1.body.token);

      expect(logOut1.body).toEqual({});
      expect(logOut1.statusCode).toEqual(200);

      const createQuizLoggedOut = requestAdminQuizCreate(token1.body.token, 'Matthew Vucic', 'Soccer');
      const createQuizLoggedIn = requestAdminQuizCreate(token2.body.token, 'Matthew Vucic', 'Basketball');
  ...
  describe('Error message appropriately returned', () => {
    test('Token empty', () => {
      const logout = requestAdminAuthLogout('');
    ...
    test('Token invalid', () => {
      ...
      const logOut1 = requestAdminAuthLogout(token1.token);
      const logOut2 = requestAdminAuthLogout(token1.token);
