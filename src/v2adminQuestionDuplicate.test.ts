import {
  ...
  requestClear, v2requestAdminQuestionDuplicate, v2requestAdminQuestionCreate
...
describe('adminQuizInfo', () => {
  describe('Error Tests', () => {
    test('Invalid token', () => {
      ...
      const questionDuplicate = v2requestAdminQuestionDuplicate(quiz.quizId, question.questionId, user.token + 'invalid');
    ...
    test('Invalid quizId', () => {
      ...
      const question = v2requestAdminQuestionCreate(quiz.quizId, user.token, questionBody).body;
      const questionDuplicate = v2requestAdminQuestionDuplicate(quiz.quizId + 1, question.questionId, user.token);
    ...
    test('Quiz not owned by user', () => {
      ...
      const question = v2requestAdminQuestionCreate(quiz.quizId, user.token, questionBody).body;
      const questionDuplicate = v2requestAdminQuestionDuplicate(quiz.quizId, question.questionId, user2.token);
    ...
    test('Invalid questionId', () => {
      const user = requestAdminAuthRegister('matthewvucic@gmail.com', 'Password1', 'Matthew', 'Vucic').body;
      const quiz = v2requestAdminQuizCreate(user.token, 'Aero', 'Quiz about Aeroplanes').body;
      const questionDuplicate = v2requestAdminQuestionDuplicate(quiz.quizId, 1, user.token);
  ...
  describe('Passing Tests', () => {
    test('Successful call', () => {
      ...
      const questionDuplicate = v2requestAdminQuestionDuplicate(quiz.quizId, question.questionId, user.token);
